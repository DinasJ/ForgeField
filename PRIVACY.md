...
ForgeField ("the extension") is a browser extension that connects Endfield (SKPort) with Google Apps Script and Discord for automation and notifications. This policy describes what data the extension uses and where it is stored.
...
- **SKPort / Endfield:** The extension reads your game session from the SKPort website (via cookies and in-page storage) when you have the site open, so it can show connection status and optionally trigger automation. We do not send this data to any server we control.
